function [c,x,y] = datamap_jc(xyz,dx,dy,v,cax,ramp,edge,usermap)
%Function [c,x,y] = datamap_jc(XYZ,dX,dY,V,Cax,Ramp,Edge,Usermap) - PCOLOR data with land map.
...
if v(2)-v(1)<=7 | v(4)-v(3)<=7, dxdy=1; end  % edit for minimum dx/dy cocient [jcedeno]

% Plot the land mask
if nargin <= 7
    if maplast == 1;
        hold on, coastmap(v,1,dxdy); else, hold on, coastmap(v,'nofill',dxdy); end
elseif nargin == 8
    if maplast == 1;
        hold on, coastmap(v,1,dxdy,usermap); else, hold on, coastmap(v,'nofill',dxdy,usermap); end
end
